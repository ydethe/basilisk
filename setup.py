...
try:
    ...
    if conan_version < "1.40.1":
        print(
            ...
            + "conan version "
            + conan_version
            + " is not compatible with Basilisk."
        )
        print(
            "use version 1.40.1+ to work with the conan repo changes from 2021."
            + endColor
        )
        exit(0)
    if conan_version > "1.59.0":
        ...
        print(
            "use version 1.40.1 to 1.59.0 to work with the conan repo changes."
            + endColor
        )
        exit(0)
    ...
    print(
        "Please make sure you install python conan package\nRun command `pip install conan` "
        "for Windows\nRun command `pip3 install conan` for Linux/MacOS"
    )
    sys.exit(1)

# define BSK module option list (option name and default value)
bskModuleOptionsBool = {"opNav": False, "vizInterface": True, "buildProject": True}
bskModuleOptionsString = {"autoKey": "", "pathToExternalModules": ""}
bskModuleOptionsFlag = {"clean": False, "allOptPkg": False}
...
class BasiliskConan(ConanFile):
    ...
    def system_requirements(self):
        ...
        if not (sys.version_info.major == 3 and sys.version_info.minor >= 7):
            print(
                warningColor
                + "Python 3.7 or newer should be used with Basilisk."
                + endColor
            )
            print(
    ...
    def configure(self):
        ...
        if self.settings.build_type == "Debug":
            print(
                warningColor
                + "Build type is set to Debug. Performance will be significantly lower."
                + endColor
            )
        ...
        if self.options.generator == "":
            ...
            if self.settings.os == "Macos":
                ...
                print(
                    "Specify your own using the -o generator='Name' flag during conan install"
                )
        else:
    ...
    def generateMessageModules(self, originalWorkingDirectory):
        ...
        if self.options.pathToExternalModules:
            cmdString.extend(
                ["--pathToExternalModules", str(self.options.pathToExternalModules)]
            )
    ...
    def build(self):
        ...
        if self.options.pathToExternalModules:
            print(
                statusColor
                + "Including External Folder: "
                + endColor
                + str(self.options.pathToExternalModules)
            )
        ...
        if self.settings.compiler == "Visual Studio":
            cmake.definitions["CONAN_LINK_RUNTIME_MULTI"] = cmake.definitions[
                "CONAN_LINK_RUNTIME"
            ]
        ...
        cmake.definitions["PYTHON_VERSION"] = (
            sys.version_info.major + 0.1 * sys.version_info.minor
        )
        cmake.parallel = True
        ...
        if self.options.buildProject:
            ...
            if self.generator == "Xcode":
                ...
                cmake.build(
                    ["--", "-jobs", str(tools.cpu_count()), "-parallelizeTargets"]
                )
            else:
                cmake.build()
            ...
            if self.settings.os != "Linux":
                print(
                    f"{statusColor}Please open project file inside dist3 with {self.generator} IDE "
                    f"and build the project for {self.settings.build_type}{endColor}"
                )
            else:
                print(
                    f"{statusColor}Please go to dist3 folder and run command "
    ...
    def add_basilisk_to_sys_path(self):
        ...
        process = subprocess.Popen(
            add_basilisk_module_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        output, err = process.communicate()
        if err:
            print(
                "Error %s while running %s"
                % (err.decode(), add_basilisk_module_command)
            )
            sys.exit(1)
        else:
...
def conan_build():
    ...
    for opt, value in bskModuleOptionsBool.items():
        parser.add_argument(
            "--" + opt,
            help="build modules for " + opt + " behavior",
            default=value,
            type=lambda x: (str(x).lower() == "true"),
        )
    for opt, value in bskModuleOptionsString.items():
        parser.add_argument(
            "--" + opt, help="using string option for " + opt, default=value
        )
    for opt, value in bskModuleOptionsFlag.items():
        parser.add_argument(
            "--" + opt, help="using flag option for " + opt, action="store_true"
        )
        ...
        if str(vars(args)[opt]):
            if opt == "pathToExternalModules":
                ...
                if os.path.exists(externalPath):
                    ...
                    print(
                        f"{failColor}Error: path {str(vars(args)[opt])} does not exist{endColor}"
                    )
                    sys.exit(1)
            else:
                conanCmdString.append(" -o " + opt + "=" + str(vars(args)[opt]))
...
if __name__ == "__main__":
    setup(
        ...
        cmdclass={"bdist_wheel": BuildConanExt},
    )
